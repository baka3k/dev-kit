...
3. Run `hi-repo-recon` with `deep` depth and `hi-tech-build-audit` with `deep` depth. May run concurrently with isolated outputs. Direct graph calls in this skill stay limited to inventory reads, `query_subgraph` spot-checks, and `reconstruct_flow` on already-verified paths per [dev-shared/graph-function-selection.md](../dev-shared/graph-function-selection.md) (T3 profile, mostly delegated to the leaves in step 6).
