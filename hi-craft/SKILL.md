...
| --parallel, "parallel" | parallel | Delegate one `implementer` per plan phase (delegation contract) |
...
| --parallel | Per phase (delegate) | Skip | Run command |
...
* Use `hi-sequential-thinking` to analyze the task briefly.
...
Parallel mode (`--parallel`): delegate one `implementer` per phase per [dev-shared/delegation-contract.md](../dev-shared/delegation-contract.md).
...
Run the `reviewer` role ([dev-shared/roles/reviewer.md](../dev-shared/roles/reviewer.md); delegate or inline per [dev-shared/delegation-contract.md](../dev-shared/delegation-contract.md)). Score>=9.5 + 0 critical = auto-approve (auto mode only).
